    ...
    // Query tanpa LIMIT dan OFFSET (karena MariaDB tidak mendukung parameter untuk LIMIT)
    $stmt = $pdo->prepare("SELECT date, check_in, check_out, status, location, notes, shift
                        FROM attendance 
                        WHERE user_id = ? AND date BETWEEN ? AND ?
                        ORDER BY date DESC");
    $stmt->execute([$user_id, $start, $end]);
    $allMonthly = $stmt->fetchAll();

    // Ambil hanya data untuk halaman saat ini
    $monthly = array_slice($allMonthly, $offset, $limit);

    // Hitung total data untuk pagination
    $totalData = count($allMonthly);
                            ...
                            <form method="post">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select name="shift" required class="w-full border rounded-lg p-2" id="shiftSelect">
                                        ...
                                        <option value="Morning" data-hint="Morning shift: 07:30 - 11:59">Pagi</option>
                                        <option value="Afternoon" data-hint="Afternoon shift: 13:00 - 17:30">Siang</option>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" class="py-4 text-center text-gray-500">No attendance records found for this month.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
